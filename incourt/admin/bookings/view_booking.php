...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT  b.*,concat(c.fullname) as client from `booking_list` b inner join client_list c on b.client_id = c.id where b.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<fieldset class="border-bottom">
    ...
    <fieldset class="bor">
        ...
        <dl>
            ...
            <dt class="">Hours</dt>
            <dd class="pl-4"><?= isset($hours) ? $hours : "" ?></dd>
            <dt class="">Fee</dt>
            <dd class="pl-4"><?= isset($price) ? $price * $hours : "" ?></dd>
            <dt class="">Status</dt>
            <dd class="pl-4">
                <?php 
                    switch($status){
